...
void INS_Task(void)
{
    ...
    /* 第一次计算程序开始了多长时间，之后就是计算上一次运行到这里到这一次之间的时间，并将当前DWT计数值保存到INS_DWT_Count */
    ...
    if ((count % 1) == 0)
    {
        ...
        // 机体系基向量（b）转换到导航坐标系（n），本例选取惯性系为导航系
        ...
        Aim_Tx.Quaternions.w = INS.q[0];
        Aim_Tx.Quaternions.x = INS.q[1];
        Aim_Tx.Quaternions.y = INS.q[2];
        Aim_Tx.Quaternions.z = INS.q[3];
